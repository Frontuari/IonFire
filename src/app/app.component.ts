  ...
  constructor(platform: Platform, 
    ...
    authService.authState.subscribe(user => {
      if(user){
        ...
        this.uaList = database.list('user-activity').map(_userActivities => _userActivities.filter(userActivity => userActivity.uid == this.user.uid)) as FirebaseListObservable<UserActivity[]>;
        //  Search if have data
        this.uaList.subscribe(userActivities => {
          if (userActivities.length > 0) {
            lastUAPos = (userActivities.length-1);
            ...
            let lastDate = moment(uaAllData[lastUAPos].d_fecha,'YYYY-MM-DD');
            ...
            if(differenceDate != j){
              ...
              if(differenceDate > 0){
                ...
                for(let i =0; i<differenceDate; i++){
                  ...
                  let curDate = getCurDate(new Date(uaAllData[lastUAPos].d_fecha),i+1,'+').toISOString().slice(0, 10);
